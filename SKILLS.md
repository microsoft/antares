...
- To get the total dimension size of a tensor, e.g. `SPLIT` shown in the example below, please use: size_of_SPLIT(). Please note that size_of_XXX() is not a compile-time constant, so consider using `__builtin_assume` instead of `static_assert`.
