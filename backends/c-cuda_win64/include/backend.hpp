...
//; eval_flags(c-cuda_win64): [x86_64-w64-mingw32-g++] -O2 -static
...
namespace ab {
  ...
  void memcpyHtoD(void *dptr, void *hptr, size_t byteSize) {
    ...
    CHECK(0 == cuMemcpyHtoDAsync_v2(dptr, hptr, byteSize, nullptr), "Failed to copy memory to device.");
  }

  void memcpyDtoH(void *hptr, void *dptr, size_t byteSize) {
    LOAD_ONCE(cuMemcpyDtoHAsync_v2, int (*)(...));
    CHECK(0 == cuMemcpyDtoHAsync_v2(hptr, dptr, byteSize, nullptr), "Failed to copy memory from device.");
  }

  void synchronize() {
    LOAD_ONCE(cuStreamSynchronize, int (*)(void*));
    CHECK(0 == cuStreamSynchronize(nullptr), "Failed to sychronize default stream.");
  }
  ...
  void* recordTime() {
    ...
    CHECK(0 == cuEventRecord(hEvent, nullptr), "Failed to record event.");
    return hEvent;
  }
