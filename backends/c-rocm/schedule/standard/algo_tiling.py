...
def schedule_branch(attrs, output, prefix):
  ...
  if attrs.is_tuning:
    ...
    assert num_threads == 1 or num_threads >= min(num_elements, 64), "Unrecommended small thread counts: %d" % num_threads
  ...
  for i, load in enumerate(load_stage):
    ...
    fused_o, fused_i = s[load].split(fused_o, factor=1)
  ...
  # unroll
  unroll_step = cfg.define_knob(f"{prefix}S", [0, 4, 32, 512])
