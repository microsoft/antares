...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < inputs.size(); ++i) {
        ...
        auto shape = parse_shape(args[0]);
        assert(args[1] == "float32");

        auto &tensor = feed_dict[args[2]];
        // tensor = g.addConstant<T>(dtype, shapes, hptr);
        tensor = g.addVariable(poplar::FLOAT, poplar::ArrayRef<std::size_t>(shape), args[2]);
        place_tensor(g, tensor);

        auto istream = g.addHostToDeviceFIFO("write_" + args[2], FLOAT, tensor.numElements());
        prog.add(poplar::program::Copy(istream, tensor));
        auto &h_data = host_data[args[2]];
        h_data.resize(tensor.numElements());
        for (int c = 0; c < h_data.size(); ++c)
            h_data[c] = (1 + i + c) % 71;

        printf("KernelInfo: Input(%s) = (shape: ", args[2].c_str());
        for (auto &it: shape)
            printf(" %ld,", it);
        printf(");\n");
    }
    ...
    auto shape = parse_shape(args[0]);
    auto output_name = args[2];
    printf("KernelInfo: Output(%s) = (shape: ", output_name.c_str());
    for (auto &it: shape)
        printf(" %ld,", it);
    printf(");\n");
    assert(args[1] == "float32");
    ...
    for (int it = 0; it < shape.size(); ++it) {
        auto str = get_between(source, "axis_" + std::to_string(it) + "\": [-1, ", "]");
        if (str.size() > 0)
          nparts.push_back(std::atoi(str.c_str()));
        else
          nparts.push_back(1);
    ...
    std::string function_name = "Vuid_" + get_between(source, "\nclass Vuid_", ":");
